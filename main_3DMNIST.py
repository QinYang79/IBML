...
from models.basic_model_2D3D import TTClassifier
...
def train_epoch(args, epoch, model, device, dataloader, optimizer, scheduler, writer=None,logger=None, scale=10):
    ...
    for step, (spec, image, anoise, vnoise, label, ids) in enumerate(dataloader):
        ...
        if args.modulation_starts < epoch < args.modulation_ends and args.add_noise == 1: 
            ...
            if aa > 1: 
                p =  np.clip(aa, 0, e)/e
                mask =  ((torch.rand(bs).to(device) < p).float() * pred1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand_as(spec) 
        ...
        optimizer.zero_grad()
        a, v, out  = model.module.forward(spec.float(), image.float())
...
def main():
    ...
    train_dataset = MNISTDataset(partition='train')
    test_dataset = MNISTDataset(partition='test')
    
    if args.dataset == ['avsbench','AVE']:
        val_dataset = MNISTDataset(partition='test')
    else:
    ...
    model = TTClassifier(args,n_classes=10)
    model.to(device)
