...
from dataset.ModelNet40trainset import TripletDataloader 
from dataset.ModelNet40testset import TestDataloader
from models.basic_model_2D3D import ModelNetClassifier
...
def train_epoch(args, epoch, model, device, dataloader, optimizer, scheduler, writer=None,logger=None, scale=10):
    ...
    global soft_labels, iter_, r, last_raitos, stds
    ...
    for step, (spec, spec2, spec3, spec4, image, anoise, vnoise, label, ids) in enumerate(dataloader):
        iter_ += 1
        ...
        spec3 = spec3.to(device)
        spec4 = spec4.to(device)
        ...
        if args.modulation_starts < epoch < args.modulation_ends and args.add_noise == 1: 
            a_noise = torch.normal(0, spec.std(), size= spec[0].size()).cuda().unsqueeze(0).repeat(bs,1,1,1) 
            v_noise = torch.normal(0, image.std(), size= image[0].size()).cuda().unsqueeze(0).repeat(bs,1,1) 

            if aa > 1: 
                p =  np.clip(aa, 0, e)/e
                mask_1 =  ((torch.rand(bs).to(device) < p).float() * pred1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand_as(spec) 
                mask_3 =  ((torch.rand(bs).to(device) < p).float() * pred1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand_as(spec) 
                spec = spec + a_noise * mask_1
                spec2 = spec2 + a_noise * mask_3

            if av > 1:  
                p =  np.clip(av, 0, e)/e  
                mask =  ((torch.rand(bs).to(device) < p).float() * pred2).unsqueeze(-1).unsqueeze(-1).expand_as(image)  
                image = image + v_noise * mask

        optimizer.zero_grad()
        a, v, out  = model.module.forward(spec.float(), spec2.float(), spec3.float(), spec4.float(), image.float())
...
def valid(args, model, device, dataloader):
    ...
    with torch.no_grad():
        ...
        for step, (spec, spec2, spec3, spec4, image, anoise, vnoise, label, ids) in enumerate(dataloader):
            ...
            a, v, out = model(spec.float(),spec2.float(),spec3.float(),spec4.float(), image.float())
...
def main():
    ...
    train_dataset = TripletDataloader(dataset_dir='./data/ModelNet40/')
    test_dataset = TestDataloader(dataset_dir='./data/ModelNet40/')
    ...
    model = ModelNetClassifier(args,n_classes=40)
    model.to(device)
    ...
    if args.train:
        ...
        for epoch in range(args.epochs):
            ...
            if acc > best_acc:
                ...
                logger.info("Loss: {:.3f}, Loss_2d: {:.3f}, Loss_3d: {:.3f}, Loss_f: {:.3f}, Best Acc: {:.3f}".format(batch_loss, batch_loss_a, batch_loss_v,batch_loss_f, best_acc))
                logger.info("2D Acc: {:.3f}, 3D Acc: {:.3f} Fusion Acc: {:.3f}".format(acc_a, acc_v, acc))
            else:
        ...
        logger.info('Accuracy: {}, accuracy_2d: {}, accuracy_3d: {}'.format(acc, acc_a, acc_v))
        ...
        try:
            ...
            logger.info('Accuracy: {}, accuracy_2d: {}, accuracy_3d: {}'.format(acc_last, acc_a_last, acc_v_last))
